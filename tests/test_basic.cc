...
int main() {
    ...
    // 为级别 4 的所有日志注册两个回调
    auto idA = logger.AddCallback(callbackA, vgp::Logger::Level::INFO);
    auto idB = logger.AddCallback(callbackB, vgp::Logger::Level::INFO);
    ...
    logger.AddCallback(
        ...
        vgp::Logger::Level::INFO, "Important message"
    );
    logger.AddCallback(
        [](const vgp::LogContext& context) {
    ...
    // 记录一些日志
    int i = 999;
    VGP_LOG(vgp::Logger::Level::INFO, "This is a debug message {}\n", i);
    VGP_LOG(vgp::Logger::Level::INFO, "Important message\n");

    // 使用默认格式（只打印消息）
    logger.RemoveCallback(idA);
    VGP_INFO("4 This is an info message\n");

    logger.ClearCallbacks(vgp::Logger::Level::INFO);

    // 切换到带时间的格式
    vgp::Logger::GetInstance().SetFormat(vgp::Logger::Format::kMedium);
    std::vector<int> arr = {1,2,3,4};
    VGP_LOGN(vgp::Logger::Level::DEBUG, "5 This is a debug message with time: {}", arr);
    VGP_INFO("4 This is a info message with time\n");
    VGP_LOGN(vgp::Logger::Level::ERROR, "2 This is a warn message with time");

    // 切换到默认格式
    vgp::Logger::GetInstance().SetFormat(vgp::Logger::Format::kFull);
    VGP_LOGN(vgp::Logger::Level::FATAL, "1 This is an error message with default format");

    // 将日志输出重定向到文件，并使用只有消息的格式
    //Logger::GetInstance().SetLogFile("app.log", true);
    vgp::Logger::GetInstance().SetFormat(vgp::Logger::Format::kLite);
    VGP_LOGFN(vgp::Logger::Level::ERROR, "2 This is a warning message in file");
    vgp::Logger::GetInstance().SetFormat(vgp::Logger::Format::kMedium);
    VGP_LOGFN(vgp::Logger::Level::DEBUG, "5 This is a debug message in file with time");
    VGP_LOGF(vgp::Logger::Level::INFO, "4 This is a info message in file with time\n");
    VGP_LOGFN(vgp::Logger::Level::WARN, "2 This is a warn message in file with time");
   ...
   // print to console
    ...
    VGP_CLOGFN(vgp::Logger::Level::INFO, "this is one compile log in file");
